...
fn geometry_path() -> Option<PathBuf> {
    // Reuse the same PulseBar config directory as Settings, so remembered
    // geometry lives alongside `settings.json` on every platform.
    pulsebar::settings::Settings::settings_path()
...
pub fn save_entry(key: &str, geometry: StoredGeometry) {
    ...
    if let Err(err) = save_file(&file) {
        tracing::warn!(target: "pulsebar::geometry", %err, "failed to persist geometry");
    }
}
