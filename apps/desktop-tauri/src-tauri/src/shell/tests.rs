...
use super::transition::{
    SurfaceSnapshot, TransitionResolution, current_surface_snapshot, hidden_surface_snapshot,
    ...
    should_defer_tray_panel_reveal, should_hide_tray_panel_on_toggle,
    should_run_tray_panel_reveal_fallback, should_synthesize_default_position,
};
...
#[test]
fn transition_serial_try_lock_reports_busy_without_blocking() {
    let serial = std::sync::Mutex::new(());
    let _guard = serial.lock().unwrap();

    let result =
        super::try_lock_transition_serial(&serial).expect("local mutex should not be poisoned");
...
#[test]
fn same_mode_reopen_keeps_frontend_surface_snapshot_unchanged() {
    let mut state = AppState::new();
    state.transition_surface(SurfaceMode::TrayPanel, SurfaceTarget::Summary);
...
#[test]
fn entering_tray_panel_defers_reveal_until_frontend_layout() {
    ...
    assert!(should_defer_tray_panel_reveal(&transition));
}

#[test]
fn same_mode_tray_reopen_uses_native_show() {
    let transition = SurfaceTransition {
...
#[test]
fn reveal_fallback_fires_only_for_still_hidden_tray_panel() {
    // Surface is still TrayPanel and the window is not yet visible: the
    // frontend reveal was starved, so the Rust fallback must force the show.
    assert!(should_run_tray_panel_reveal_fallback(
        SurfaceMode::TrayPanel,
        false
    ));
    // Frontend fast path already revealed it: never double-show.
    assert!(!should_run_tray_panel_reveal_fallback(
        SurfaceMode::TrayPanel,
        true
    ));
    // Surface moved on (hidden/settings/popout): never resurrect the panel.
    assert!(!should_run_tray_panel_reveal_fallback(
        SurfaceMode::Hidden,
        false
    ));
