...
/// Remove the borderless subclass so a decorated window regains its native
/// frame (title bar, caption buttons, resize border).
///
/// The subclass is installed once on the reused `main` window at startup and
/// forces every surface frameless via `WM_NCCALCSIZE`. When the shell switches
/// that window into a *decorated* mode (Pop-Out / Settings), the subclass would
/// otherwise keep suppressing the non-client area — so it must be removed and
/// the frame recalculated. Idempotent: a no-op when no subclass is installed.
#[cfg(windows)]
pub fn remove_dark_caption(win: &tauri::WebviewWindow) {
    use raw_window_handle::HasWindowHandle;
    ...
    unsafe {
        let removed =
            RemoveWindowSubclass(hwnd, borderless_subclass_proc, BORDERLESS_SUBCLASS_ID) != 0;

        // Undo the "sheet of glass" frame extension so the restored native
        ...
        // Force a frame recalculation now that the subclass no longer zeroes
        // the non-client area.
        const SWP_FRAMECHANGED: u32 = 0x0020;
        const SWP_NOMOVE: u32 = 0x0002;
        const SWP_NOSIZE: u32 = 0x0001;
        ...
        tracing::info!("dwm: removed borderless subclass (was_installed={removed})");
    }
}
...
#[cfg(not(windows))]
pub fn remove_dark_caption(_win: &tauri::WebviewWindow) {}
