...
/// Reanchor position for the tray panel using its CURRENT measured size (after
/// the frontend resizes it) on the tray's OWN monitor.
///
/// Mirrors [`tray_panel_position`]'s monitor resolution — pick the monitor
/// containing the tray anchor — but takes the post-resize panel size instead of
/// the fixed default. Falls back to the bottom-right of the primary work area
/// only when there is no tray anchor. This is what keeps the panel on the
/// tray's monitor instead of teleporting to the primary one after every open.
pub fn reanchor_tray_panel_position(
    app: &AppHandle,
    ...
    {
        let scale = monitor.scale_factor();
        return Some(window_positioner::calculate_panel_position(
            &tray_anchor_rect(anchor),
            &monitor_work_area_rect(monitor),
            &panel_size,
            scale,
    ...
    // No anchor (or its monitor was unplugged): bottom-right of the primary work
    // area, matching the previous reanchor fallback.
    let monitor = window
        .primary_monitor()
        .ok()
        .flatten()
