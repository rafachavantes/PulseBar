...
use super::{ShellTransitionRequest, lock_shell_transition_serial};
...
pub(super) fn should_defer_tray_panel_reveal(transition: &SurfaceTransition) -> bool {
    transition.to == SurfaceMode::TrayPanel && transition.from != SurfaceMode::TrayPanel
}

/// How long the Rust fallback waits for the frontend to reveal the tray panel
/// before forcing the window visible itself.
const TRAY_PANEL_REVEAL_FALLBACK_DELAY: std::time::Duration = std::time::Duration::from_millis(300);

/// Whether the deferred Rust-side reveal fallback should force the tray panel
/// window visible: only when the surface is still `TrayPanel` and the window
/// has not already been shown by the frontend fast path. Pure so it can be
/// unit-tested without a live window.
pub(super) fn should_run_tray_panel_reveal_fallback(mode: SurfaceMode, is_visible: bool) -> bool {
    mode == SurfaceMode::TrayPanel && !is_visible
}

/// Guarantee the tray panel becomes visible even if the frontend reveal path is
/// starved. The frontend normally reveals the window after its first layout
/// pass; if that never runs (its effect early-returns, or its measurement run
/// is cancelled by layout churn) the window would otherwise stay hidden
/// forever — the `REOPEN-FAILS` symptom.
fn spawn_tray_panel_reveal_fallback(app: &AppHandle) {
    let app = app.clone();
    tauri::async_runtime::spawn(async move {
        tokio::time::sleep(TRAY_PANEL_REVEAL_FALLBACK_DELAY).await;
        ...
        if should_run_tray_panel_reveal_fallback(mode, is_visible) {
            tracing::info!(
                ?mode,
                "shell: tray-panel reveal fallback firing (frontend reveal did not run)"
            );
            ...
            tracing::debug!(
                ?mode,
                is_visible,
                "shell: tray-panel reveal fallback skipped (already shown or mode changed)"
            );
        }
    });
}
...
fn apply_same_mode_target_update(
    ...
    tracing::debug!(from = ?mode, to = ?mode, "shell: emit surface-mode-changed (retarget)");
...
pub(super) fn apply_transition(
    ...
    match apply_window_layout(window, &transition.properties) {
        Ok(needs_show) => {
            let defer_tray_panel_reveal = should_defer_tray_panel_reveal(transition);
            ...
            tracing::debug!(
                ...
                to = ?transition.to,
                "shell: emit surface-mode-changed"
            );
            ...
            tracing::debug!(
                ...
                to = ?transition.to,
                needs_show,
                defer = defer_tray_panel_reveal,
                "shell: apply_transition show decision"
            );
            if needs_show {
                if defer_tray_panel_reveal {
                    // Fast path is the frontend reveal (see useTrayPanelLayout).
                    // Guarantee visibility with a Rust-side fallback in case the
                    // JS reveal is starved and never fires.
                    spawn_tray_panel_reveal_fallback(app);
                } else {
                    let _ = show_window(window);
                }
            }
