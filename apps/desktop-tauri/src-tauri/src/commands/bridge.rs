...
impl RateWindowSnapshot {
    ...
    fn with_pace_reserve(mut self, pace: &pulsebar::core::UsagePace) -> Self {
...
pub(crate) fn pace_stage_str(stage: pulsebar::core::PaceStage) -> &'static str {
...
impl ProviderUsageSnapshot {
    pub(super) fn from_fetch_result(
        ...
        let primary_pace = pulsebar::core::UsagePace::weekly(&usage.primary, None, 10080);
...
pub(crate) fn friendly_provider_error(id: ProviderId, error: &str) -> String {
    ...
    if lower.contains("claude oauth credentials not found") {
        return "Claude sign-in was not found. Run `claude` once to authenticate, then refresh Claude in PulseBar.".to_string();
    }

    if lower.contains("oauth token expired") || lower.contains("token invalid or expired") {
        return "Claude sign-in expired. Run `claude` to refresh your Claude Code login, then refresh Claude in PulseBar.".to_string();
    }

    if trimmed == "Authentication required" {
        return "Claude needs sign-in before PulseBar can read usage. Run `claude` once, or add Claude cookies in Provider settings.".to_string();
    }
...
impl From<Settings> for SettingsSnapshot {
    fn from(settings: Settings) -> Self {
        ...
        Self {
            ...
            menu_content_mode: settings.menu_content_mode,
...
pub(crate) fn bridge_commands() -> Vec<BridgeCommandDescriptor> {
    vec![
        ...
        BridgeCommandDescriptor {
            ...
            description: "Return browsers detected on this machine that PulseBar can import cookies from.",
        },
        BridgeCommandDescriptor {
...
pub(crate) fn bridge_events() -> Vec<BridgeEventDescriptor> {
    vec![
        ...
        BridgeEventDescriptor {
            id: "settings-changed",
            description: "Emitted when persisted settings change. Payload: full settings snapshot.",
        },
    ]
}
