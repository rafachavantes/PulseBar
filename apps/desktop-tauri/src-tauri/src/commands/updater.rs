...
use pulsebar::updater::UpdateInfo;
...
pub async fn check_for_updates(
    ...
    let result = tokio::time::timeout(
        ...
        pulsebar::updater::check_for_updates_with_channel(settings.update_channel),
    )
    .await;
...
fn spawn_download_task(app_handle: tauri::AppHandle, info: UpdateInfo) {
    tokio::spawn(async move {
        let (tx, rx) = tokio::sync::watch::channel(pulsebar::updater::UpdateState::Available);
...
fn spawn_download_progress_task(
    app: tauri::AppHandle,
    mut rx: tokio::sync::watch::Receiver<pulsebar::updater::UpdateState>,
) -> tokio::task::JoinHandle<()> {
    tokio::spawn(async move {
        while rx.changed().await.is_ok() {
            let backend_state = rx.borrow().clone();
            if let pulsebar::updater::UpdateState::Downloading(progress) = backend_state {
...
async fn run_download_task(
    app: tauri::AppHandle,
    info: UpdateInfo,
    tx: tokio::sync::watch::Sender<pulsebar::updater::UpdateState>,
) -> UpdateStatePayload {
    let download_handle =
        tokio::spawn(async move { pulsebar::updater::download_update(&info, tx).await });
...
pub fn apply_update(state: tauri::State<'_, Mutex<AppState>>) -> Result<(), String> {
    ...
    pulsebar::updater::verify_installer_hash(&path, &expected_sha256)?;
    pulsebar::updater::apply_update(&path)
}
