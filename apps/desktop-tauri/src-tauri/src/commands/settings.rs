...
pub struct SettingsUpdate {
    ...
    pub menu_content_mode: Option<String>,
...
impl SettingsUpdate {
    ...
    fn apply_display_settings(self, settings: &mut Settings) -> Self {
        ...
        if let Some(v) = self.menu_content_mode.as_deref() {
            settings.menu_content_mode = pulsebar::settings::normalize_menu_content_mode(v);
        }
...
pub async fn update_settings(
    ...
    // A changed usage/cookie source (or provider set) can change chart data, so
    // drop the memoized bundles; the next open rescans with the new settings.
    super::chart::invalidate_chart_cache();
    ...
    let _ = app.emit(events::SETTINGS_CHANGED, &snapshot);
...
mod tests {
    ...
    #[test]
    fn menu_content_mode_patch_applies_and_normalizes() {
        let mut settings = Settings::default();
        assert_eq!(settings.menu_content_mode, "lean");

        SettingsUpdate {
            menu_content_mode: Some("full".to_string()),
            ..Default::default()
        }
        .apply_display_settings(&mut settings);
        assert_eq!(settings.menu_content_mode, "full");

        SettingsUpdate {
            menu_content_mode: Some("bogus".to_string()),
            ..Default::default()
        }
        .apply_display_settings(&mut settings);
        assert_eq!(settings.menu_content_mode, "lean");
