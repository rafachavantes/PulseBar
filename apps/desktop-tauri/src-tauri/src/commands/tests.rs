...
use pulsebar::host::session::launch_block_reason;
use pulsebar::settings::{ApiKeys, Language, ManualCookies, Settings};
...
fn external_url_validation_allows_only_http_urls() {
    assert_eq!(
        validate_external_url(" https://github.com/rafachavantes/PulseBar ").unwrap(),
        "https://github.com/rafachavantes/PulseBar"
    );
    assert_eq!(
        validate_external_url("http://github.com/rafachavantes/PulseBar").unwrap(),
        "http://github.com/rafachavantes/PulseBar"
    );
...
fn bootstrap_contract_lists_surface_mode_changed_event() {
    ...
    assert!(ids.contains(&"settings-changed"));
}
...
fn credential_status_labels_do_not_include_error_details() {
    assert_eq!(
        super::credential_file_status_label(pulsebar::secure_file::SecureFileStatus::Missing),
        "missing"
    );
    assert_eq!(
        super::credential_file_status_label(pulsebar::secure_file::SecureFileStatus::Plaintext),
        "plaintext"
    );
    assert_eq!(
        super::credential_file_status_label(pulsebar::secure_file::SecureFileStatus::Protected(
            "windows-dpapi-user".to_string(),
        )),
        "protected:windows-dpapi-user"
    );
    assert_eq!(
        super::credential_file_status_label(pulsebar::secure_file::SecureFileStatus::Unreadable(
...
fn apply_provider_order_dedupes_and_appends_unknown_canonical() {
    ...
    let order =
        pulsebar::settings::normalize_provider_order(&["codex".to_string(), "claude".to_string()]);
    ...
    let canonical = pulsebar::core::ProviderId::all()
        .iter()
...
fn apply_provider_order_ignores_unknown_ids() {
    let order = pulsebar::settings::normalize_provider_order(&[
...
fn provider_summaries_reflect_settings_order() {
    let canonical_len = pulsebar::core::ProviderId::all().len();
...
fn provider_cache_upsert_replaces_existing_provider() {
    ...
    let result = ProviderFetchResult {
        usage: pulsebar::core::UsageSnapshot::new(pulsebar::core::RateWindow::new(10.0)),
        cost: None,
...
fn claude_transient_auth_failure_preserves_first_last_good_snapshot() {
    ...
    let result = ProviderFetchResult {
        usage: pulsebar::core::UsageSnapshot::new(pulsebar::core::RateWindow::new(42.0)),
        cost: None,
...
fn claude_error_message_explains_missing_sign_in() {
    ...
    assert_eq!(
        message,
        "Claude sign-in was not found. Run `claude` once to authenticate, then refresh Claude in PulseBar."
    );
}
...
fn open_path_rejects_missing_path() {
    let missing = std::env::temp_dir()
        .join(format!("pulsebar-phase6d-missing-{}", std::process::id()))
        .join("does-not-exist");
...
fn external_url_validator_accepts_http_and_https() {
    assert_eq!(
        super::validate_external_url(" https://github.com/rafachavantes/PulseBar "),
        Ok("https://github.com/rafachavantes/PulseBar")
    );
    assert_eq!(
