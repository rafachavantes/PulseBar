...
pub fn is_remote_session() -> Result<bool, String> {
    Ok(pulsebar::host::session::is_ssh_session() || pulsebar::host::session::is_remote_session())
}

#[tauri::command]
pub fn get_launch_block_reason() -> Result<Option<String>, String> {
    Ok(pulsebar::host::session::current_launch_block_reason().map(|s| s.to_string()))
}
...
pub fn get_work_area_rect(app: tauri::AppHandle) -> Result<WorkAreaRect, String> {
    ...
    if let Some(area) = pulsebar::host::session::primary_work_area_pixels() {
...
pub fn play_notification_sound() -> Result<(), String> {
    ...
    let settings = Settings::load();
    pulsebar::sound::play_alert(pulsebar::sound::AlertSound::Success, &settings);
    Ok(())
}

/// Reposition the tray panel so it stays anchored to the system-tray area.
/// Called from the frontend after dynamic resize.
#[tauri::command]
pub fn reanchor_tray_panel(app: tauri::AppHandle) -> Result<(), String> {
    use crate::window_positioner::PanelSize;
    use tauri::Manager;
    ...
    // Resolve the panel position on the TRAY's own monitor (the monitor
    // containing the saved tray anchor), not the primary one — otherwise a
    // non-primary taskbar makes the panel jump to the primary monitor moments
    // after opening.
    let Some((x, y)) = crate::shell::reanchor_tray_panel_position(&app, panel_size) else {
        return Ok(());
    };
...
fn dashboard_url_for_provider(provider_id: &str) -> Option<String> {
    if let Some(url) = pulsebar::settings::get_api_key_providers()
        .into_iter()
...
pub async fn trigger_provider_login(
    ...
    // runners live in `pulsebar::login` but are async-oriented and tightly
