...
pub fn reveal_tray_panel_window(
    ...
    // Poison-recover: a prior panic must not permanently break the reveal path
    // and leave the tray window stuck hidden.
    let mode = state
        .lock()
    ...
    if mode != SurfaceMode::TrayPanel {
        tracing::debug!(
            ?mode,
            "reveal_tray_panel_window: early-return (mode != TrayPanel)"
        );
        return Ok(());
    }
    ...
    tracing::debug!(?mode, ok = show.is_ok(), "reveal_tray_panel_window: show()");
    show?;
