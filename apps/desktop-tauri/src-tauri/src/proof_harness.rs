...
//!
//! Activated by the `PULSEBAR_PROOF_MODE` environment variable.  The value
...
/// Proof configuration parsed from `PULSEBAR_PROOF_MODE`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
impl ProofConfig {
    ...
    ///
    /// Returns `None` when `PULSEBAR_PROOF_MODE` is unset or empty.
    pub fn from_env() -> Option<Self> {
        let raw = std::env::var("PULSEBAR_PROOF_MODE").ok()?;
        let raw = raw.trim();
        if raw.is_empty() {
            return None;
        }
        ...
        let Some(surface_mode) = SurfaceMode::parse(surface_str) else {
            tracing::warn!("PULSEBAR_PROOF_MODE: unknown surface '{surface_str}', ignoring");
            return None;
        };

        if !proof_payload_is_supported(surface_mode, payload.as_deref()) {
            tracing::warn!("PULSEBAR_PROOF_MODE: unsupported target '{raw}', ignoring");
            return None;
        }
...
fn native_menu_snapshot_for_path(menu_path: &str) -> (String, Vec<String>) {
    ...
    let enabled = pulsebar::settings::Settings::load().enabled_providers;
...
mod tests {
    ...
    fn with_proof_mode_env(value: Option<&str>, test: impl FnOnce()) {
        ...
        let prev = std::env::var("PULSEBAR_PROOF_MODE").ok();

        match value {
            Some(value) => unsafe { std::env::set_var("PULSEBAR_PROOF_MODE", value) },
            None => unsafe { std::env::remove_var("PULSEBAR_PROOF_MODE") },
        }

        test();

        match prev {
            Some(prev) => unsafe { std::env::set_var("PULSEBAR_PROOF_MODE", prev) },
            None => unsafe { std::env::remove_var("PULSEBAR_PROOF_MODE") },
        }
    }
    ...
    fn about_path_snapshot_clears_on_failure() {
        let _guard = MENU_LOCK.lock().unwrap();
        set_menu_snapshot(Some("tray".into()), vec!["About PulseBar".into()]);
