...
describe("MenuCard", () => {
  ...
  it("lean mode keeps session, weekly and tokens but hides cost and extras", async () => {
    const resetAt = new Date(Date.now() + 6 * 24 * 60 * 60 * 1000);
    const snapshot = provider(null, 35);
    snapshot.secondary = rateWindow(62, {
      windowMinutes: 7 * 24 * 60,
      resetsAt: resetAt.toISOString(),
    });
    snapshot.tertiary = rateWindow(10);

    renderCard(snapshot, { contentMode: "lean" });

    // Session + weekly windows survive; tertiary and pace-budget pills do not.
    expect(await screen.findByText("Session")).toBeInTheDocument();
    expect(screen.getByText("DetailWindowSecondary")).toBeInTheDocument();
    expect(screen.queryByText("DetailWindowTertiary")).not.toBeInTheDocument();
    expect(screen.queryByText(/On-pace budget/)).not.toBeInTheDocument();

    // Token counts survive; cost cells and the estimate note do not.
    expect(await screen.findByText("30d tokens")).toBeInTheDocument();
    expect(screen.getByText("584K")).toBeInTheDocument();
    expect(screen.queryByText("30d cost")).not.toBeInTheDocument();
    expect(
      screen.queryByText("Estimated from local logs"),
    ).not.toBeInTheDocument();
  });
