...
function ProviderPill({
  ...
  const exhausted = Boolean(provider.primary.isExhausted || provider.error);
  // Severity always follows USED percent so a healthy account never reads as
  // critical in "show remaining" mode; thresholds honor the user's sliders.
  const severity = usageSeverity(used, exhausted, {
    high: highUsed,
    critical: critUsed,
  });
  const tone: "ok" | "warn" | "crit" =
    severity === "normal" ? "ok" : severity === "high" ? "warn" : "crit";
...
export default function FloatBar({ state }: { state: BootstrapState }) {
  ...
  // Track the OS light/dark preference so a "system" theme can flip the pill
  // palette live (light glass + dark text) without a reload.
  const [prefersLightDesktop, setPrefersLightDesktop] = useState(() => {
  ...
  useEffect(() => {
    ...
    const handle = () => setPrefersLightDesktop(mq.matches);
    handle();
  ...
  // The floatbar floats over the desktop, not the app surface, so its palette
  // can't just inherit the dark theme. Resolve a light backdrop from the
  // explicit dark-text toggle, the chosen theme, or the OS preference so the
  // dark-text/light-glass palette auto-applies instead of rendering
  // white-on-white on a light wallpaper.
  const lightBg =
    settings.floatBarDarkText ||
    settings.theme === "light" ||
    (settings.theme !== "dark" && prefersLightDesktop);

  return (
    <div
      className={`floatbar floatbar--${orientation} floatbar--${style}${lightBg ? " floatbar--light-bg" : ""}`}
      data-tauri-drag-region
      ...
      {visible.length === 0 ? (
        ...
        visible.map((p) => (
          <ProviderPill
            key={p.providerId}
            provider={p}
            highUsed={settings.highUsageThreshold}
            critUsed={settings.criticalUsageThreshold}
