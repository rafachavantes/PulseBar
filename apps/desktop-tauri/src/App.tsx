...
import { checkForUpdates, getBootstrapState, getSettingsSnapshot } from "./lib/tauri";
...
function AppInner() {
  ...
  useEffect(() => {
    ...
    // NOTE: the global shortcut is owned entirely by the native toggle handler
    // (`shortcut_bridge::plugin` + `reregister_shortcut`). We intentionally do
    // NOT listen for a `global-shortcut-triggered` force-show event here — a
    // second handler is what turned a custom shortcut into show-only.
    ...
    window.addEventListener("pulsebar:settings-updated", onSettingsUpdated);
    ...
    return () => {
      ...
      window.removeEventListener("pulsebar:settings-updated", onSettingsUpdated);
    };
  }, [reloadBootstrapState]);
