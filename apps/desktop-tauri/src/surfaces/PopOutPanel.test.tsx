...
function renderPopOut(
  ...
  return render(
    <LocaleProvider>
      <PopOutPanel state={bootstrap(catalog, settingsOverrides)} providerId={providerId} />
    </LocaleProvider>,
  );
}
...
describe("PopOutPanel", () => {
  ...
  it("shows an empty-cache state instead of saying providers are unconfigured", async () => {
    const { container, getByText, queryByText } = renderPopOut(
      [],
      undefined,
      [],
      { enabledProviders: ["codex", "claude"] },
    ...
    await waitFor(() => {
      expect(container.querySelector(".menu-surface__empty")).not.toBeNull();
    });
