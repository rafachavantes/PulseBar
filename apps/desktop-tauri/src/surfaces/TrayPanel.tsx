...
const HAS_DASHBOARD = new Set([
  "claude", "codex", "gemini", "grok", "opencodego", "synthetic", "zai",
]);
...
/**
 * Tray popover surface — two modes like macOS PulseBar:
...
export default function TrayPanel({ state }: { state: BootstrapState }) {
  ...
  const hasEnabledProviders = settings.enabledProviders.length > 0;
  ...
  // First-run empty-state CTA lands on the Providers tab (where the hint text
  // tells users to enable providers), not the General tab.
  ...
  const footerRows: MenuFooterRow[] = [
    ...
    { icon: "ⓘ", label: "About PulseBar", onClick: openAbout },
    { icon: "⌧", label: "Quit", shortcut: "Ctrl+Q", onClick: quitApp },
  ];

  // Surface the global summon shortcut so users can discover it. Hidden when
  // no shortcut is configured.
  const summonShortcut = settings.globalShortcut?.trim();
  const footerHint = summonShortcut ? (
    <div className="menu-surface__shortcut-hint">
      <span className="menu-surface__shortcut-hint-label">
        {t("TrayShortcutHintLabel")}
      </span>
      <kbd className="menu-surface__shortcut-kbd">{summonShortcut}</kbd>
    </div>
  ) : null;
      ...
      <div className={revealClassName}>
        ...
        >
          <MenuEmpty
            ...
            mode={hasEnabledProviders ? "noData" : "noProviders"}
              ...
              <Fragment key={p.providerId}>
                ...
                >
                  <MenuCard
                    ...
                    contentMode={settings.menuContentMode}
