...
describe("TrayPanel provider grid", () => {
  ...
  it("hides the tray through Rust instead of closing the main window when opening Settings", async () => {
    const close = vi.fn().mockResolvedValue(undefined);
    windowMocks.getCurrentWindow.mockReturnValue({
      setSize: vi.fn().mockResolvedValue(undefined),
      close,
    });
    const { container, getByText } = renderTrayPanel([
  ...
  it("removes disabled providers when settings change in another window", async () => {
    const { container } = renderTrayPanel(
      [provider("claude", "Claude", 35), provider("zai", "z.ai", 55)],
      { enabledProviders: ["claude", "zai"] },
    );

    await waitFor(() => {
      expect(
        container.querySelector('.provider-grid__item[aria-label="z.ai"]'),
      ).not.toBeNull();
    });

    act(() => {
      emitEvent("settings-changed", settings({ enabledProviders: ["claude"] }));
    });
  ...
  it("does not reveal the native tray window again for refresh-only updates", async () => {
    const { container } = renderTrayPanel([provider("claude", "Claude", 35)]);
  ...
  it("does not request a provider refresh just because the tray panel mounted", async () => {
    const { container } = renderTrayPanel([provider("claude", "Claude", 35)]);
  ...
  it("shows an empty-cache state instead of saying providers are unconfigured", async () => {
    const { container, getByText, queryByText } = renderTrayPanel([], {
      enabledProviders: ["codex", "claude"],
    });
    ...
    expect(getByText("StateNoProviderData")).toBeInTheDocument();
    expect(queryByText("NoProvidersConfigured")).toBeNull();

    fireEvent.click(getByText("TooltipRefresh"));
  ...
  it("does not shrink a three-provider overview to the empty minimum before measuring", async () => {
    const setSize = vi.fn().mockResolvedValue(undefined);
    windowMocks.getCurrentWindow.mockReturnValue({
      setSize,
      close: vi.fn().mockResolvedValue(undefined),
    ...
    const scrollHeightSpy = vi
      ...
      .mockImplementation(function (this: HTMLElement) {
        return this.classList.contains("menu-surface--tray") ? 460 : 0;
      });
    const rectSpy = vi
    ...
    try {
      renderTrayPanel(
        [
          provider("codex", "Codex", 25),
          provider("claude", "Claude", 35),
          provider("zai", "z.ai", 45),
        ],
        { enabledProviders: ["codex", "claude", "zai"] },
      );

      await waitFor(() => {
        expect(setSize).toHaveBeenCalledWith(
          expect.objectContaining({ width: 328, height: 464 }),
        );
      });
      expect(setSize).not.toHaveBeenCalledWith(
  ...
  it("keeps provider detail mode tall enough for context actions and footer", async () => {
    ...
    const { container } = renderTrayPanel([errorProvider], {
      enabledProviders: [errorProvider.providerId],
    });
