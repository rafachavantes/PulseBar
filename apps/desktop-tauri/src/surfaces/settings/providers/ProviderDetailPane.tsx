...
export function ProviderDetailPane({
  ...
  const needsLogin = providerNeedsLogin(detail.lastError);
  ...
  // After credentials are saved/imported/removed, re-fetch the provider detail
  // (usage, credential-storage status, etc.). The API-key and cookie sections
  // refresh their own displayed state in their save handlers, so they are NOT
  // keyed on `credentialRevision` — remounting them here would discard the
  // "Saved ✓" flash before it paints. Bumping the revision still refreshes the
  // token-accounts panel, which has no self-refresh of its own.
  ...
  return (
    <div className="provider-detail">
      ...
      {needsLogin && (
        <div className="provider-detail-signin">
          <button
            type="button"
            className="provider-detail-signin__btn"
            onClick={handleSwitchAccount}
            disabled={busy}
          >
            {t("SignInToProvider").replace("{}", detail.displayName)}
          </button>
        </div>
      )}
...
/**
 * Heuristic: does this provider's last error indicate the user simply needs to
 * sign in (vs. a transient/other fetch failure)? Shared by the issue notice,
 * the primary sign-in CTA, and the quick-action label.
 */
function providerNeedsLogin(message: string | null): boolean {
  if (!message) return false;
  const lower = message.replace(/^last fetch failed:\s*/i, "").trim().toLowerCase();
  return (
...
function ProviderIssueNotice({
  ...
  const needsLogin = providerNeedsLogin(message);
...
/** A storage-status value counts as revocable when a credential file actually
 *  exists on disk. "missing" (never created) and "unavailable" (path could not
 *  be resolved) mean there is nothing to revoke. */
function isStoredStatus(value: string): boolean {
...
function CredentialStorageSection({
  ...
  const hasRevocable =
    isStoredStatus(status.apiKeys) ||
    isStoredStatus(status.manualCookies) ||
    isStoredStatus(status.tokenAccounts);
