...
interface Props {
  providerId: string;
  /** Fired after a key is saved/removed and providers are refreshed, so the
   *  parent detail pane can re-fetch `getProviderDetail`. */
  onSaved?: () => void;
}
 ...
 */
export function ApiKeySection({ providerId, onSaved }: Props) {
  const { t } = useLocale();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setEditValue("");
      setEditLabel("");
      // Re-fetch usage with the new key and let the detail pane reload.
          ...
          {editing && (
            <div className="credential-card__edit">
              ...
              <div className="credential-card__edit-actions">
                ...
                >
                  {flash === "saving" ? t("CredentialSaving") : "Save"}
                </button>
                <button
                  className="credential-btn"
                  disabled={busy}
