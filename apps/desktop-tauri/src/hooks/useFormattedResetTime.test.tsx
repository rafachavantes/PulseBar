...
describe("useFormattedResetTime", () => {
  ...
  it("drops the zero-hour component for sub-hour windows", async () => {
    const target = new Date("2024-06-01T00:05:00Z").toISOString();
    await mountWithLocale(
      <Probe resetsAt={target} fallback="later" relative={true} />,
    );
    const node = screen.getByTestId("reset");
    expect(node).toHaveTextContent("Resets in 5m");
    expect(node).not.toHaveTextContent("0h");
  });
