param(
    ...
    [string]$InstallDir = "$env:LOCALAPPDATA\Programs\PulseBar",
...
foreach ($name in @("pulsebar", "pulsebar-desktop", "pulsebar-desktop-tauri")) {
    Get-Process -Name $name -ErrorAction SilentlyContinue | Stop-Process -Force
}

$logDir = Join-Path $env:TEMP "pulsebar-installer-smoke"
...
$exe = Join-Path $InstallDir "pulsebar.exe"
$desktopExe = Join-Path $InstallDir "pulsebar-desktop.exe"
...
Write-Step "installed pulsebar.exe sha256: $installedHash"
$desktopHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $desktopExe).Hash.ToLowerInvariant()
Write-Step "installed pulsebar-desktop.exe sha256: $desktopHash"

if ($ExpectedVersion) {
    $versionOutput = (& $exe --version) -join "`n"
    if ($LASTEXITCODE -ne 0) {
        throw "pulsebar.exe --version exited with $LASTEXITCODE"
    }
    if ($versionOutput -notmatch [regex]::Escape($ExpectedVersion)) {
        throw "Expected pulsebar.exe --version to mention $ExpectedVersion, got: $versionOutput"
    }
...
if ($LASTEXITCODE -ne 0) {
    throw "pulsebar.exe --help exited with $LASTEXITCODE"
}
if ($helpOutput -notmatch "Usage:" -or $helpOutput -notmatch "diagnose") {
    throw "pulsebar.exe --help did not print CLI help."
}
Write-Step "CLI help output: ok"

$uninstallKeys = @(
    "HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\WinPulseBar_is1",
    "HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\WinPulseBar_is1",
    "HKLM:\Software\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\WinPulseBar_is1"
)
$uninstallEntry = $null
...
if ($null -eq $uninstallEntry) {
    throw "Missing WinPulseBar uninstall registry entry."
}
...
$shortcutCandidates = @(
    (Join-Path $startMenu "PulseBar.lnk"),
    (Join-Path $startMenu "PulseBar\PulseBar.lnk")
)
