...
<#
.SYNOPSIS
    Check whether a PulseBar release is ready or complete.
...
param(
    [string]$Version = "",
    [string]$AssetsDir = "$env:TEMP\PulseBar-release\assets",
    [switch]$SkipGitHub
)
...
$tag = "v$Version"
Write-Host "Release doctor: PulseBar $Version"
Write-Host ""
...
if (Test-Path $AssetsDir) {
    Test-AssetHash (Join-Path $AssetsDir "PulseBar-$Version-Setup.exe")
    Test-AssetHash (Join-Path $AssetsDir "PulseBar-$Version-portable.exe")
} else {
...
if (-not $SkipGitHub) {
    ...
    if ($gh) {
        Push-Location $RepoRoot
        try {
            $ghJsonPath = Join-Path $env:TEMP "win-pulsebar-release-doctor-gh.json"
            $ghErrPath = Join-Path $env:TEMP "win-pulsebar-release-doctor-gh.err"
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                foreach ($name in @(
                    "PulseBar-$Version-Setup.exe",
                    "PulseBar-$Version-Setup.exe.sha256",
                    "PulseBar-$Version-portable.exe",
                    "PulseBar-$Version-portable.exe.sha256"
                )) {
