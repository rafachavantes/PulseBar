#!/usr/bin/env bash
# Build and run the PulseBar Tauri desktop shell on Linux / WSL.
#
# Usage:
...
if grep -qi microsoft /proc/version 2>/dev/null || [ -n "${WSL_DISTRO_NAME:-}" ]; then
    ...
    if [ -z "${DISPLAY:-}" ] && [ -z "${WAYLAND_DISPLAY:-}" ]; then
        ...
        if [ "$CLI_MODE" -eq 0 ]; then
            echo "Tip: 'pulsebar usage -p claude' works without a display."
            echo ""
            CLI_MODE=1
...
if [ "$SKIP_BUILD" -eq 0 ]; then
    if [ "$CLI_MODE" -eq 1 ]; then
        if [ "$RELEASE" -eq 1 ]; then
            echo "Building PulseBar CLI (release, target=$NATIVE_TARGET)..."
            cargo build --manifest-path "$RUST_DIR/Cargo.toml" --bin pulsebar --release "${TARGET_FLAG[@]}"
        else
            echo "Building PulseBar CLI (debug, target=$NATIVE_TARGET)..."
            cargo build --manifest-path "$RUST_DIR/Cargo.toml" --bin pulsebar "${TARGET_FLAG[@]}"
        fi
    else
        cd "$TAURI_APP_DIR"
        if [ "$RELEASE" -eq 1 ]; then
            echo "Building PulseBar Desktop (release, no bundle)..."
            pnpm run tauri:build
        else
            echo "Building PulseBar Desktop (debug, no bundle)..."
...
if [ "$CLI_MODE" -eq 1 ]; then
    BINARY_NAME="pulsebar"
else
    BINARY_NAME="pulsebar-desktop-tauri"
fi
...
echo ""
if [ "$CLI_MODE" -eq 1 ]; then
    echo "Running: pulsebar usage -p all"
    RUN_ARGS=(usage -p all)
else
    echo "Running: PulseBar Desktop"
fi
