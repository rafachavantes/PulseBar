...
### Added
- Ported upstream PulseBar 0.26 AWS Bedrock monthly spend tracking to the Windows/Tauri Rust backend using AWS Cost Explorer and SigV4 request signing.
...
### Changed
- Aligned the Windows/Tauri package version with upstream PulseBar 0.25.1 after reviewing the upstream patch set.
...
### Added
- Ported upstream PulseBar 0.25 provider support for Manus, Xiaomi MiMo, Doubao, Command Code, Crof, StepFun, Venice, and OpenAI API balance to the Windows/Tauri Rust backend.
...
### Fixed
- Bundle Microsoft's Evergreen WebView2 bootstrapper in the Inno installer and install WebView2 Runtime silently on clean Windows machines before launching PulseBar.
...
### Added
- Ported upstream PulseBar 0.24 provider support for Codebuff, DeepSeek, and Windsurf to the Windows/Tauri Rust backend.
...
### Fixed
- Fix Windows CLI visibility - `pulsebar.exe usage` now produces output in PowerShell (Issue #22)
...
### Fixed
- Aligned the CLI help text with the actual supported provider set so `nanogpt` now appears in both top-level help and `pulsebar usage --help`.
...
- Replaced the stale root Swift CI and Linux release workflows with Rust-native automation that validates and packages the `pulsebar` binary from `rust/`.
...
### Changed
- The Windows installer now bundles Microsoft's Visual C++ redistributable and installs it before launching PulseBar on clean machines.

### Fixed
- Fixed clean-Windows installer runs failing to launch PulseBar because the required Visual C++ runtime was missing.
...
Complete porting of Swift PulseBar features to Rust Windows version:
...
### Added
- Initial Windows port of PulseBar using Rust + egui
