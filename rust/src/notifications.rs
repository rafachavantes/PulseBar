//! System notifications for PulseBar
//!
...
impl NotificationManager {
    ...
    fn show_toast(&self, title: &str, body: &str) {
        ...
        // CreateToastNotifier("PulseBar") finds a valid registration rather than
...
'@
    ...
    $notifier = [Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier("PulseBar")
    if ($null -eq $notifier) {{ throw "CreateToastNotifier returned null" }}
    $notifier.Show($toast)
}} catch {{
    [System.Console]::Error.WriteLine("PulseBar toast failed: $_")
    exit 1
}}"#,
...
/// Register the PulseBar App User Model ID (AUMID) in the Windows registry so that
/// `CreateToastNotifier("PulseBar")` resolves to a valid notifier instead of returning
...
fn ensure_aumid_registered() {
    ...
    let result = hkcu
        .create_subkey(r"SOFTWARE\Classes\AppUserModelId\PulseBar")
        .and_then(|(key, _)| key.set_value("DisplayName", &"PulseBar"));

    match result {
        Ok(()) => tracing::debug!("PulseBar AUMID registered for Windows toast notifications"),
        Err(e) => tracing::warn!("Failed to register PulseBar AUMID: {}", e),
    }
}
