...
mod tests {
    ...
    fn parses_current_codex_payload_token_count_events() {
        let path = std::env::temp_dir().join(format!(
            "pulsebar-current-codex-token-count-{}.jsonl",
            std::process::id()
        ));
        let mut file = File::create(&path).unwrap();
        // Timestamp must fall inside the scanner's rolling window (now - N days),
        // so it is generated relative to now instead of hardcoded.
        let timestamp = (Utc::now() - Duration::days(1)).format("%Y-%m-%dT%H:%M:%S%.3fZ");
        writeln!(
            file,
            r#"{{"timestamp":"{timestamp}","type":"event_msg","payload":{{"type":"token_count","info":{{"model":"gpt-5","total_token_usage":{{"input_tokens":125,"cached_input_tokens":30,"output_tokens":15}}}}}}}}"#
        )
        .unwrap();
        drop(file);
