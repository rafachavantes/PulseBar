...
fn launch_log_path() -> PathBuf {
    std::env::temp_dir().join(format!("pulsebar_launch_{}.log", std::process::id()))
}
...
fn missing_subcommand() -> i32 {
    ...
    eprintln!(
        "pulsebar is now CLI-only. Run a subcommand (e.g. `pulsebar usage -p claude`) \
         or launch the Tauri desktop shell via `apps/desktop-tauri`.\n\
         Use `pulsebar --help` for the full list of subcommands."
    );
...
mod tests {
    ...
    fn launch_log_path_is_process_scoped() {
        ...
        assert!(file_name.starts_with("pulsebar_launch_"));
