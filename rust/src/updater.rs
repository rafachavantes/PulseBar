//! Auto-update checker for PulseBar
...
const GITHUB_REPO: &str = "rafachavantes/PulseBar";
...
fn get_download_dir() -> Option<PathBuf> {
    dirs::cache_dir().map(|p| p.join("PulseBar").join("updates"))
}
...
mod tests {
    ...
    fn prefers_installer_asset_for_auto_update() {
        let release = GitHubRelease {
            ...
            html_url: "https://github.com/rafachavantes/PulseBar/releases/tag/v1.2.6".to_string(),
            body: None,
            assets: vec![
                GitHubAsset {
                    name: "pulsebar.exe".to_string(),
                    browser_download_url: "https://example.com/pulsebar.exe".to_string(),
                    digest: None,
                },
                GitHubAsset {
                    name: "PulseBar-1.2.6-Setup.exe".to_string(),
                    browser_download_url: "https://example.com/PulseBar-1.2.6-Setup.exe"
                        .to_string(),
                    digest: Some(
        ...
        assert_eq!(
            update.download_url,
            "https://example.com/PulseBar-1.2.6-Setup.exe"
        );
    ...
    fn falls_back_to_manual_release_when_only_portable_exe_exists() {
        ...
        assert_eq!(
            update.download_url,
            "https://github.com/rafachavantes/PulseBar/releases/tag/v1.2.6"
        );
        assert!(!update.supports_auto_apply());
    }
    ...
    fn finds_newest_pending_installer_and_ignores_portable_exe() {
        ...
        let portable = temp.path().join("pulsebar.exe");
        let older = temp
            .path()
            .join(format!("PulseBar-{}.{}.{}-Setup.exe", major, minor, patch));
    ...
    fn verify_installer_hash_accepts_matching_sha256() {
        let temp = tempfile::tempdir().expect("temp dir");
        let path = temp.path().join("PulseBar-1.2.3-Setup.exe");
