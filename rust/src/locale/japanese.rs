...
impl LocaleKey {
    pub(super) fn japanese(self) -> &'static str {
        match self {
            ...
            LocaleKey::TrayTooltipMoreProviders => "他 {} 件",

            // Onboarding UX — first-run empty state + summon shortcut hint
            LocaleKey::EmptyStateHeadline => "すべての AI 使用量をひと目で。",
            LocaleKey::EmptyStateProvidersCaption => {
                "Codex・Claude・Gemini・z.ai・Grok・OpenCode Go に対応"
            }
            LocaleKey::TrayShortcutHintLabel => "呼び出し",

            // Onboarding UX — credential save feedback
            LocaleKey::CredentialSaving => "保存中…",
            LocaleKey::CredentialSaved => "保存しました ✓",

            // Onboarding UX — provider sign-in visibility
            LocaleKey::ActionSignIn => "サインイン",
            LocaleKey::SignInToProvider => "{} にサインイン",
        }
    }
}
