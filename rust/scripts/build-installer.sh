...
INSTALLER_PATH="$OUTPUT_DIR/PulseBar-${VERSION}-Setup.exe"
INNO_IMAGE="${INNO_SETUP_IMAGE:-amake/innosetup}"
CONTAINER_NAME="pulsebar-inno-${VERSION//./-}-$$"
...
for required_file in "$TARGET_BIN_DIR/pulsebar.exe" "$RUST_DIR/icons/icon.ico"; do
...
docker create \
  ...
  --entrypoint /bin/sh \
  "$INNO_IMAGE" \
  -lc "/opt/bin/iscc /Qp /DAppVersion=$VERSION /DTargetBinDir=..\\\\target\\\\$TARGET_TRIPLE\\\\release /DVCRedistPath=..\\\\target\\\\installer-deps\\\\vc_redist.x64.exe /DWebView2BootstrapperPath=..\\\\target\\\\installer-deps\\\\MicrosoftEdgeWebview2Setup.exe /DOutputDir=C:\\\\inno-out /DOutputBaseFilename=PulseBar-$VERSION-Setup pulsebar.iss" \
  >/dev/null

docker start -a "$CONTAINER_NAME" >/dev/null
docker cp \
  "$CONTAINER_NAME:/home/xclient/.wine/drive_c/inno-out/PulseBar-$VERSION-Setup.exe" \
  "$INSTALLER_PATH"
