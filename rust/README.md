# PulseBar for Windows

A Windows port of [PulseBar](https://github.com/steipete/CodexBar) - a system tray application for monitoring AI provider usage limits.

![PulseBar Windows](screenshots/tray-icon.png)
...
Download the latest release from the [Releases](https://github.com/Finesssee/Win-PulseBar/releases) page.

- Recommended: `PulseBar-<version>-Setup.exe`
  - This installer now installs the required Microsoft Visual C++ runtime and Microsoft Edge WebView2 Runtime on clean Windows machines before launching PulseBar.
- Portable: `PulseBar-<version>-portable.exe`
...
- Linux CLI: `pulsebar-v<version>-linux-x86_64.tar.gz` or `pulsebar-v<version>-linux-aarch64.tar.gz`
  - Includes the standalone `pulsebar` CLI binary plus README and license files.
...
git clone https://github.com/Finesssee/Win-PulseBar.git
cd Win-PulseBar/rust

# Build release version
cargo build --release

# Run the CLI
./target/release/pulsebar.exe --help
...
- Use `General -> Updates` to choose the release channel, background auto-download behavior, and whether a ready installer should run automatically when you quit PulseBar
...
On Windows Remote Desktop sessions, PulseBar now exits with a direct error dialog instead of crashing with renderer errors. Use the local Windows desktop session for the native UI, or run CLI commands like `pulsebar usage -p claude` while connected over RDP.
...
pulsebar autostart enable
pulsebar autostart disable
pulsebar autostart status
```
...
Settings are stored in `%APPDATA%\PulseBar\settings.json`:
...
Manual cookies, API keys, token accounts, and settings are stored under `%APPDATA%\PulseBar`.
...
This is a Windows port of [PulseBar](https://github.com/steipete/CodexBar) by [Peter Steinberger](https://twitter.com/steipete).
