...
<#
.SYNOPSIS
    Build and run the PulseBar Tauri desktop shell for Windows.
...
$DesktopBinaryName = "pulsebar-desktop-tauri.exe"
...
if (-not $SkipBuild) {
    ...
    try {
        if ($Release) {
            Write-Host "Building PulseBar Desktop (release, no bundle)..." -ForegroundColor Cyan
            & $pnpmCommand.Source run tauri:build
        } else {
            Write-Host "Building PulseBar Desktop (debug, no bundle)..." -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Starting PulseBar Desktop..." -ForegroundColor Green
& $binary
