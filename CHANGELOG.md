...
### Verified (on Linux)
- Backend `cargo test -p pulsebar`: 240 tests pass; `cargo clippy` clean.
...
### Changed
- Port upstream PulseBar 0.33.0 provider and cost-accounting fixes into the Windows/Tauri Rust backend.
...
### Changed
- Install `pulsebar.exe` as the console CLI and `pulsebar-desktop.exe` as the tray app so Start Menu shortcuts launch the desktop UI while terminal diagnostics print real output.
...
### Changed
- Port upstream PulseBar 0.31.0 provider behavior into the Windows/Tauri Rust backend while keeping macOS-only AppKit menu and Homebrew changes out of the Windows shell.
...
### Added
- Add `pulsebar diagnose`, a generic safe provider diagnostic export that reports provider/source/config/fetch health without exposing cookies, tokens, account emails, or raw secrets.

### Changed
- Port upstream PulseBar 0.30.1 provider diagnostics behavior to the Rust CLI while omitting macOS-only AppKit status-item handling that has no Windows Tauri equivalent.
...
### Added
- Port upstream PulseBar 0.29 Alibaba Token Plan support to the Rust/Tauri provider registry, protected token-account storage, provider icon registry, and CLI aliases.
...
### Added
- Port upstream PulseBar 0.28 Azure OpenAI and T3 Chat provider support to the Windows/Tauri provider registry, Settings UI metadata, protected credential storage, and CLI aliases.
...
### Added
- Complete the upstream PulseBar 0.27 provider port for Windows/Tauri by adding Grok billing support, Claude Admin API usage, OpenAI Admin API usage with legacy credit-balance fallback, MiniMax billing summaries, OpenCode Go Zen balance display, and Kiro overage usage/cost parsing.
...
- Add `pulsebar serve` for loopback `/health`, `/usage`, and `/cost` JSON, with loopback Host-header validation.
...
### Added
- Port upstream PulseBar 0.27 API quota providers for ElevenLabs subscription credits, Deepgram project usage, GroqCloud Enterprise Prometheus metrics, and LLM Proxy quota-stats.
...
- Add `pulsebar config providers`, `pulsebar config enable`, `pulsebar config disable`, and `pulsebar config set-api-key` for scriptable provider setup.
...
### Added
- Port upstream PulseBar 0.26 AWS Bedrock monthly spend tracking into the Windows/Tauri Rust backend using AWS Cost Explorer and SigV4 request signing.
...
- Restore portable release packaging to a standalone `PulseBar-<version>-portable.exe` asset instead of a zip bundle.
...
### Changed
- Align the Windows/Tauri release with upstream PulseBar 0.25.1 after reviewing the upstream patch set.
...
### Added
- Port upstream PulseBar 0.25 provider support for Manus, Xiaomi MiMo, Doubao, Command Code, Crof, StepFun, Venice, and OpenAI API balance into the Windows/Tauri app.
...
### Added
- Port upstream PulseBar 0.24 provider support for Codebuff, DeepSeek, and Windsurf into the Windows/Tauri app.
...
### Fixed
- Ship `WebView2Loader.dll` beside `pulsebar.exe` in the Windows installer so clean installs can launch the Tauri shell.
- Replace the standalone portable executable release asset with `PulseBar-<version>-portable.zip`, which includes both `pulsebar.exe` and `WebView2Loader.dll`.
- Add release workflow checks that fail the build when the WebView2 runtime sidecar is missing.

### Superseded
- Later Windows MSVC builds statically link the WebView2 loader, so release packaging no longer needs to ship `WebView2Loader.dll` beside `pulsebar.exe`.
...
- Release workflow now builds Tauri app as pulsebar.exe
...
### Preferences & UI
- Windows: open the main window automatically when tray startup is unavailable, and support `PULSEBAR_START_VISIBLE` for proof/automation flows.
...
- CLI: add `pulsebar cost` to print local cost usage (text/JSON) for Codex + Claude.
...
- macOS: PulseBar now supports Intel Macs (x86_64 builds + Sonoma fallbacks). Thanks @epoyraz!
...
- Homebrew: cask now installs the `pulsebar` CLI symlink. Thanks @dalisoft!
...
- Widgets: add a new “PulseBar Switcher” widget that lets you switch providers and remember the selection.
...
- CI: build/test Linux `PulseBarCLI` (x86_64 + aarch64) and publish release assets as `PulseBarCLI-<tag>-linux-<arch>.tar.gz` (+ `.sha256`).
...
## 0.8.0 — 2025-12-17
- PulseBar is now available via Homebrew: `brew install --cask steipete/tap/pulsebar` (updates via `brew upgrade --cask steipete/tap/pulsebar`).
...
## 0.6.0 — 2025-12-04
- New bundled CLI (`pulsebar`) with single `usage` command, `--format text|json`, `--status`, and fast `-h/-V`.
- CLI output now shows consistent headers (`Codex 0.x.y (codex-cli)`, `Claude Code <ver> (claude)`) and JSON includes `source` + `status`.
- Advanced prefs install button symlinks `pulsebar` into /usr/local/bin and /opt/homebrew/bin; docs refreshed.
...
- Debug replay toggle (`defaults write com.steipete.pulsebar debugMenuEnabled -bool YES`) to view every pattern.
...
- Notarized release build "PulseBar-0.1.0.zip" remains current artifact; app version 0.1.1.

## 0.1.0 — 2025-11-16
- Initial PulseBar release: macOS 15+ menu bar app, no Dock icon.
