...
Thanks for helping improve PulseBar. This repo is the Windows/Tauri port, so prefer the active Tauri and Rust codepaths over historical upstream macOS material.
...
- `rust\src\` contains shared backend/domain logic and the standalone `pulsebar` CLI.
...
- Route new provider construction through `pulsebar::core::instantiate_provider`.
